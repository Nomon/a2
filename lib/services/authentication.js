...
Authentication.prototype.renderLoginForm = function(req, res) {
  ...
  if(self.server.options.login_template) {
    res.render(self.server.options.login_template, {client_id: req.a2.client_id, redirect_uri:req.url});
  } else {
    return res.redirect(self.server.options.login_endpoint+'?redirect_uri='+redir+'&client_id='+req.a2.client.id);
  }
};
