...
/***
 * In browser window is global, exports window.A2. Makes it easy to unit test this.
 */
(function(exports, global) {
  ...
  function accessToken(token) {
    ...
    if (token === null) {
      return storage.removeItem("A2.token");
    } else if (token === undefined) {
      return storage.getItem("A2.token");
    } else {
      return storage.setItem("A2.token", token);
    }
  }
  ...
  A2.initialize = function(cfg) {
    ...
    options.site = cfg.site || options.site;
  ...
  Auth.prototype.connect = function(callback) {
    ...
    var url = this.url = new URL(options.site+"/connect");
    var params = {
