...
function secure(options) {
  ...
  var requireToken = options.require_token || true;
  if(loadUser) {
    requireToken = true;
  }
  ...
  return function(req, res, next) {
    ...
    console.log("user",req.a2.user_id);
    utils.getTokenFromRequest(req, res, function(err, tokenData) {
      ...
      } else if(!err && tokenData && req.query && req.query.client_id) {
        if(tokenData.client_id != req.a2.client_id) {
          console.log("Client ",req.query.client_id, "trying to use token issued to",tokenData.client_id);
          return errors.resource.access_denied(req, res);
        }
      }
      if(tokenData) {
        ...
        if(requireToken || (loadClient && !req.query.client_id)) {
          return errors.resource.access_denied(req, res);
        } else {
