 ...
 *  `user_service`            Service to be used in loading users from access_tokens
        ...
        if(Object.keys(scopes).length > 0) {
          ...
          return self.emit('authorize_form', req, res, scopes, function() {
            ...
            if(req.query.response_type == "code") {
              ...
              self.tokenService.createOrFind(client.client_id, user.user_id, function(error, token) {
                if(error) {
                  return errors.server_error(req, res);
                }
                console.log("Redirecting to ",client.redirect_uri+'#access_token='+encodeURIComponent(token));
