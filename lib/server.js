  ...
  this.clientService.load(query.client_id, function(err, client) {
    ...
    self.authenticationService.login(req, res, function(err, user) {
      if(err) {
        ...
        if(self.loginTemplate) {
          return res.render(self.loginTemplate, {redirect_uri:req.url});
        }
        return res.redirect(self.options.login_endpoint+'?redirect_uri='+url+'&client_id='+query.client_id);
      }
      req.a2.user = user;
        ...
        if(Object.keys(scopes).length > 0) {
          ...
          return self.emit('authorize_form', req, res, scopes, function() {
            ...
            if(req.query.response_type == "code") {
              ...
              self.tokenService.createOrFind(user.id, client.id, {scope:scopes}, function(error, token) {
                if(error) {
                  return errors.server_error(req, res);
                }
