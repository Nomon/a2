...
function Server(options) {
  ...
  this.router = exports.router = express.router(function (app) {
    ...
    app.get(options.login_endpoint, middleware.validate(self),self.loginEndpoint.bind(self));
...
Server.prototype.authorizeGET = function(req, res, next) {
  ...
  this.clientService.load(query.client_id, function(err, client) {
    ...
    self.authenticationService.login(req, function(err, user) {
      if(err) {
        return self.viewService.login(req);
      }
    });
  });
